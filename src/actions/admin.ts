...
export async function setPasswordByAdmin(id: number, typed: string): Promise<ActionResult & { password?: string }> {
  ...
  // A "I forgot my password" request from this employee is now handled.
  run("UPDATE password_reset_requests SET status = 'done', handled_by = ?, handled_at = ? WHERE user_id = ? AND status = 'pending'", me.id, Date.now(), id);
